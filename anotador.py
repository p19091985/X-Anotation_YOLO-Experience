...
class AnnotationManager:
    ...
    @staticmethod
    def load_annotations(label_path: str, image_size: Tuple[int, int]) -> Tuple[
        List[Dict[str, Any]], Optional[Exception]]:
        annotations = []
    ...
    @staticmethod
    def convert_to_yolo_format(class_id: int, box_coords: Tuple[float, float, float, float],
...
class CanvasController:
    ...
    def canvas_to_world(self, cx: float, cy: float) -> Tuple[float, float]:
        canvas_w, canvas_h = self.canvas.winfo_width(), self.canvas.winfo_height()
        img_w, img_h = self.app_state.original_image_size
        if img_w == 0 or img_h == 0 or self.zoom_level == 0: return 0, 0
        center_x_canvas = canvas_w / 2 + self.pan_offset[0]
        center_y_canvas = canvas_h / 2 + self.pan_offset[1]
    ...
    def display_image(self) -> None:
        ...
        if not self.app_state.current_pil_image: return
    ...
    def _draw_handles(self) -> None:
        ...
        coords_canvas = [self.world_to_canvas(x1w, y1w), self.world_to_canvas(x2w, y1w), self.world_to_canvas(x1w, y2w),
                         self.world_to_canvas(x2w, y2w)]
        s = 5
        tags_map = ["handle_nw", "handle_ne", "handle_sw", "handle_se"]
        for i, (px, py) in enumerate(coords_canvas):
            self.canvas.create_rectangle(px - s, py - s, px + s, py + s, fill=Config.HIGHLIGHT_COLOR, outline="black",
    ...
    def on_canvas_press(self, event: tk.Event, on_update_callback: callable) -> None:
        if self.app_state.is_drawing:
            ...
            self.temp_text = self.canvas.create_text(event.x + 5, event.y - 10, anchor='w', text="",
                                                     fill=Config.DIMENSION_TEXT_COLOR)
            return
    ...
    def on_canvas_release(self, event: tk.Event, on_update_callback: callable) -> None:
        if self.app_state.is_drawing:
            ...
            if self.draw_start_pos is None: return
            x1_c, y1_c = self.draw_start_pos
            x2_c, y2_c = event.x, event.y
            self.draw_start_pos = None
            if abs(x2_c - x1_c) < 5 and abs(y2_c - y1_c) < 5: return
        ...
        if self.action_mode and self.app_state.selected_annotation_index is not None:
            ...
            yolo_str = AnnotationManager.convert_to_yolo_format(self.app_state.annotations[idx]['class_id'],
                                                                (x1, y1, x2, y2), self.app_state.original_image_size)
    ...
    def on_mouse_hover(self, event: tk.Event) -> None:
        if self.action_mode or self.pan_start_pos: return
        cursor = ""
        if self.app_state.is_drawing:
            cursor = "crosshair"
        else:
...
class UIManager:
    ...
    def _create_widgets(self) -> None:
        ...
        theme_switch = ttk.Checkbutton(top_frame, text="Tema Escuro", variable=self.theme_var,
                                       command=self.app.toggle_theme, onvalue='dark', offvalue='light',
        ...
        self.annotation_listbox = tk.Listbox(annotations_frame, exportselection=False, borderwidth=0,
        ...
        self.add_box_check = ttk.Checkbutton(actions_frame, text="Adicionar Caixa (B)", variable=self.drawing_mode_var,
                                             command=self.app.toggle_drawing_mode, style="Switch.TCheckbutton")
        ...
        self.change_class_button = ttk.Button(class_edit_frame, text="Alterar",
                                              command=self.app.change_annotation_class)
    ...
    def set_edit_controls_state(self, state: str) -> None:
        ...
        if state == 'disabled': self.class_id_var.set("")
    ...
    def refresh_annotation_list(self) -> None:
        ...
        for ann in self.app_state.annotations:
            class_name = self.app_state.class_names[ann['class_id']] if ann['class_id'] < len(
                self.app_state.class_names) else "ID?"
    ...
    def sync_ui_to_state(self) -> None:
        ...
        if self.app_state.current_image_index != -1:
            ...
            self.status_label.config(
                text=f"Imagem {self.app_state.current_image_index + 1} de {len(self.app_state.image_paths)}")
        else:
...
class MainApplication:
    ...
    def _verify_dataset_structure(self, path: str) -> bool:
        """Verifica se a pasta contém os subdiretórios 'train', 'test' e 'valid'."""
    ...
    def _bind_events(self) -> None:
        ...
        self.ui.canvas.bind("<ButtonPress-1>",
                            lambda e: self.canvas_controller.on_canvas_press(e, self.process_canvas_update))
        self.ui.canvas.bind("<B1-Motion>", self.canvas_controller.on_canvas_drag)
        self.ui.canvas.bind("<ButtonRelease-1>",
                            lambda e: self.canvas_controller.on_canvas_release(e, self.process_canvas_update))
    ...
    def select_directory(self) -> None:
        ...
        if not self._verify_dataset_structure(path):
            messagebox.showerror("Estrutura Inválida",
                                 "A pasta selecionada deve conter os subdiretórios 'train', 'test' e 'valid'.\n\nO programa será encerrado.")
            self.root.destroy()
            return
    ...
    def _load_directory_contents(self) -> None:
        ...
        for root_dir, _, files in os.walk(self.app_state.base_directory):
            if os.path.basename(root_dir).startswith(('.', '_')): continue
        ...
        if self.app_state.image_paths:
            ...
            self.ui.update_status_bar(
                f"{len(self.app_state.image_paths)} imagens carregadas. Use as setas para navegar.")
        else:
    ...
    def show_image_at_index(self, index: int) -> None:
        if not (0 <= index < len(self.app_state.image_paths)): return
        self.deselect_all()
        ...
        self.app_state.annotations, error = self.ann_manager.load_annotations(label_path,
    ...
    def process_canvas_update(self, **kwargs) -> None:
        if kwargs.get('select_annotation_idx') is not None: self._select_annotation(kwargs['select_annotation_idx'])
        if kwargs.get('deselect_all'): self.deselect_all()
        if kwargs.get('toggle_draw_mode'): self.toggle_drawing_mode()
        if kwargs.get('add_new_box'): self._add_new_box_at(kwargs['add_new_box'])
        if kwargs.get('save_and_refresh'): self._save_and_refresh(
            new_selection=self.app_state.selected_annotation_index)

    def _select_annotation(self, index: int) -> None:
        if not (0 <= index < len(self.app_state.annotations)): return
        self.deselect_all()
    ...
    def _save_and_refresh(self, new_selection: Optional[int] = None) -> None:
        if self.app_state.current_image_index == -1: return
        image_path = self.app_state.get_current_image_path()
        if not image_path: return
        label_path = self.ann_manager.get_label_path(image_path)
    ...
    def change_annotation_class(self) -> None:
        if self.app_state.selected_annotation_index is None or not self.app_state.class_names: return
        selected_class_name = self.ui.class_id_var.get()
        if not selected_class_name: return
        try:
    ...
    def on_annotation_select_from_list(self, event: tk.Event) -> None:
        ...
        if selections: self._select_annotation(selections[0])
    ...
    def _select_adj_annotation(self, direction: int) -> None:
        if not self.ui.annotation_listbox.size(): return
    ...
    def _ask_for_class_id(self) -> Optional[int]:
        if not self.app_state.class_names:
            return simpledialog.askinteger("ID da Classe", "Digite o ID para a nova caixa:", parent=self.root,
                                           minvalue=0)
        ...
        if self.app_state.class_names: combo.set(self.app_state.class_names[0])
    ...
    def toggle_theme(self, update_config: bool = True) -> None:
        try:
            ...
            if update_config: self._save_config()
...
class ClassManagerWindow(Toplevel):
    def __init__(self, master, class_list: List[str], callback: callable):
        ...
        for item in self.class_list: self.listbox.insert(tk.END, item)
        self.listbox.bind("<Double-1>", self.edit_item)
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill=tk.X, pady=5)
        ttk.Button(button_frame, text="Adicionar", command=self.add_item).pack(side=tk.LEFT, expand=True, fill=tk.X,
                                                                               padx=2)
        ttk.Button(button_frame, text="Renomear", command=self.edit_item).pack(side=tk.LEFT, expand=True, fill=tk.X,
                                                                               padx=2)
        ttk.Button(button_frame, text="Excluir", command=self.delete_item).pack(side=tk.LEFT, expand=True, fill=tk.X,
                                                                                padx=2)
    ...
    def edit_item(self, event=None) -> None:
        ...
        new_name = simpledialog.askstring("Renomear Classe", f"Novo nome para '{old_name}':", initialvalue=old_name,
                                          parent=self)
